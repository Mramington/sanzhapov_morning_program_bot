...
commands = (
    ...
    ('more_about_program', 'Больше информации о программе'),
...
@bot.message_handler(commands=['more_about_program'])
def get_more_about_program(message):
    bot.send_message(
        chat_id=message.from_user.id,
        text='Про какую привычку вы хотите узнать подробнее',
...
def more_about_what_to_do(call):
    text = data.get('about habit4').get('details').get(call.data)
    bot.edit_message_text(
        chat_id=call.from_user.id,
        message_id=call.message.message_id,
        text=text,
